...
import it.gov.pagopa.pu.classification.exception.ClassificationExceptionHandler;
import it.gov.pagopa.pu.classification.exception.transcoder.handler.ConstraintViolationExceptionMessageTranscoderTest;
import it.gov.pagopa.pu.classification.util.TestUtils;
import it.gov.pagopa.pu.classification.util.UtilitiesTest;
...
import static it.gov.pagopa.pu.classification.exception.transcoder.handler.ConstraintViolationExceptionMessageTranscoderTest.EXPECTED_CONSTRAINT_EXCEPTION_MESSAGE_TRANSCODED;
import static org.mockito.ArgumentMatchers.any;
import static org.mockito.Mockito.doThrow;

@ExtendWith({SpringExtension.class})
@WebMvcTest(value = {CommonExceptionHandlerTest.TestController.class, CommonExceptionHandlerTest.TestCrudController.class})
@AutoConfigureMockMvc(addFilters = false)
@ContextConfiguration(classes = {
  CommonExceptionHandlerTest.TestController.class,
  CommonExceptionHandlerTest.TestCrudController.class,
...
public abstract class CommonExceptionHandlerTest {
  ...
  @MockitoSpyBean
  protected TestCrudController testCrudControllerSpy;
  @MockitoSpyBean
  ...
  @RestController
  @Slf4j
  protected static class TestCrudController {
  ...
  void handleForbiddenErrorExceptionError() throws Exception {
    ...
    performRequest(DATA, MediaType.APPLICATION_JSON)
      ...
      .andExpect(MockMvcResultMatchers.jsonPath("$.category").value("CLASSIFICATION_FORBIDDEN"))
      .andExpect(MockMvcResultMatchers.jsonPath("$.code").value("ERRORCODE"))
      .andExpect(MockMvcResultMatchers.jsonPath("$.message").value("Error"))
  ...
  void handleNotAuthorizedExceptionError() throws Exception {
    ...
    performRequest(DATA, MediaType.APPLICATION_JSON)
      ...
      .andExpect(MockMvcResultMatchers.jsonPath("$.category").value("CLASSIFICATION_UNAUTHORIZED"))
      .andExpect(MockMvcResultMatchers.jsonPath("$.code").value("ERRORCODE"))
      .andExpect(MockMvcResultMatchers.jsonPath("$.message").value("Error"))
      .andExpect(MockMvcResultMatchers.jsonPath("$.fields").doesNotExist())
      .andExpect(MockMvcResultMatchers.jsonPath("$.traceId").value(traceId));
  ...
  void handleAuthorizationDeniedException() throws Exception {
    ...
    performRequest(DATA, MediaType.APPLICATION_JSON)
      .andExpect(MockMvcResultMatchers.status().isForbidden())
      .andExpect(MockMvcResultMatchers.jsonPath("$.category").value("CLASSIFICATION_FORBIDDEN"))
      .andExpect(MockMvcResultMatchers.jsonPath("$.code").value("CLASSIFICATION_FORBIDDEN"))
      .andExpect(MockMvcResultMatchers.jsonPath("$.message").value("Error"))
      .andExpect(MockMvcResultMatchers.jsonPath("$.fields").doesNotExist())
      .andExpect(MockMvcResultMatchers.jsonPath("$.traceId").value(traceId));
  }
...
//region Spring Data Rest
  ...
  void handleCrudResourceNotFoundException() throws Exception {
    ...
    mockMvc.perform(MockMvcRequestBuilders.get("/crud/installments/-12"))
      ...
      .andExpect(MockMvcResultMatchers.jsonPath("$.message").value("EntityRepresentationModel not found"))
      .andExpect(MockMvcResultMatchers.jsonPath("$.fields").doesNotExist())
      .andExpect(MockMvcResultMatchers.jsonPath("$.traceId").value(traceId));
  }
  ...
  void handleCrudResourceBaseBusinessException() throws Exception {
    Long id = -12L;
    doThrow(new InvalidValueException("ERROR", "message", List.of(new ErrorFieldDTO().field("fieldName").error("resolvedError").message("resolved message")))).when(testCrudControllerSpy).testCrudEndpoint(id);
    ...
    mockMvc.perform(MockMvcRequestBuilders.get("/crud/installments/-12"))
      ...
      .andExpect(MockMvcResultMatchers.jsonPath("$.fields[0].field").value("fieldName"))
      .andExpect(MockMvcResultMatchers.jsonPath("$.fields[0].error").value("resolvedError"))
